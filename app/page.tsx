...
const sectionsBase: MenuSection[] = [
...
const sections = sectionsBase.map(section => ({...section, items: section.items.map((item, index) => ({...item, id: `${section.name}-${index}-${item.name}`}))}))
...
export default function Page() {
  ...
  const orderText = `Hola Káva, quiero pedir:\n${cartItems.map(item => { const quantity = cart[item.id] || 0; const subtotal = item.price * quantity; return `${quantity}x ${item.name} | Unitario: Bs ${item.price} | Subtotal: Bs ${subtotal}` }).join('\n')}\nTotal: Bs ${total}${comment ? `\nComentarios: ${comment}` : ''}`
  const submitOrder = () => {
