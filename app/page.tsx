...
export default function Page() {
  ...
  return <main className="kava-app">
    ...
    <section id="passport" className="passport page-shell"><div><p className="eyebrow">Kava Pasaporte</p><h2>Tu café,<br/><em>tu ritual.</em></h2><p>Descubre nuevos orígenes, acumula sellos y deja que cada taza te lleve un poco más lejos.</p><button className="button-primary" onClick={openPassport}>Reservar pasaporte <ArrowRight size={15}/></button></div><div className="passport-card"><div className="stamp"><Leaf size={24}/><span>KAVA</span></div><div><p className="small-label">Pasaporte de café</p><p className="passport-title">Sellos = Café</p></div><div className="stamp-row">{[1,2,3,4,5,6,7,8].map(i => <span key={i} className={i < 4 ? 'stamp-dot filled' : 'stamp-dot'}>{i < 4 ? '✓' : i}</span>)}</div></div></section>
