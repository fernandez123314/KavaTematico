...
export default function Page() {
  ...
  const orderText = `Hola Káva, quiero pedir:\n${cartItems.map(item => `${cart[item.id]}x ${item.name} - Bs ${item.price * (cart[item.id] || 0)}`).join('\n')}\nTotal: Bs ${total}${comment ? `\nComentarios: ${comment}` : ''}`
  const submitOrder = () => {
