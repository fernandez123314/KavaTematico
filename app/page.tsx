...
function BrandMark() { return <div className="brand"><div className="brand-mark"><Leaf size={25} strokeWidth={1.2}/></div><div><div className="brand-name">KAVA</div><div className="brand-sub">ECO COFFEE</div><div className="brand-branch">Sucursal Temático</div></div></div> }
...
export default function Page() {
  ...
  const submitPassport = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    window.setTimeout(() => {
      const message = `Hola Kava, quiero reservar mi Pasaporte Coffee Week\n\nNombre: ${name}\nTeléfono: ${phone}\n\nQuiero reservar mi Pasaporte Coffee Week`
  ...
  return <main className="kava-app">
    ...
    {screen === 'welcome' ? <section className="welcome page-shell"><Botanical/><div className="welcome-copy"><p className="eyebrow">Café de especialidad · sostenible y consciente</p><h1>Bienvenido<br/>a <em>Kava</em></h1><p className="hero-text">Una pausa bien hecha. Café honesto, ingredientes vivos y un espacio para volver a ti.</p><div className="welcome-actions"><button className="button-primary" onClick={openMenu}>Ver menú <ArrowRight size={15}/></button><button className="button-secondary" onClick={() => { document.getElementById('passport')?.scrollIntoView({behavior:'smooth'}); openPassport() }}>Reservar pasaporte <ArrowRight size={15}/></button><button className="button-secondary" onClick={() => document.getElementById('location')?.scrollIntoView({behavior:'smooth'})}>Ver ubicación <MapPin size={15}/></button></div><a className="whatsapp-link" href="https://wa.me/59178100028" target="_blank" rel="noreferrer">◉ Escríbenos por WhatsApp</a></div><div className="hero-note"><Leaf size={15}/> 100% sostenible</div></section> : <section id="menu" className="menu-section page-shell"><button className="back-link" onClick={() => setScreen('welcome')}><ArrowLeft size={15}/> Volver a bienvenida</button><div className="section-heading"><div><p className="eyebrow">La carta completa</p><h2>Hecho para disfrutar<br/><em>sin prisa.</em></h2></div><button className="cart-trigger" onClick={openCart}><ShoppingBag size={17}/> Tu carrito {cartCount > 0 && <b>{cartCount}</b>}</button></div><div className="category-row">{categories.map(category => <button key={category} className={active === category ? 'category-active' : ''} onClick={() => setActive(category)}>{category}</button>)}</div><div className="menu-sections">{visibleSections.map(section => <section className="menu-category" key={section.name}><div className="category-heading"><div><p className="eyebrow">Kava selección</p><h3>{section.name}</h3></div>{section.note && <p>{section.note}</p>}</div><div className="menu-grid">{section.items.map(item => <article className="menu-card" key={`${section.name}-${item.name}`}><div className="card-top"><span className="item-icon"><Leaf size={16}/></span><span className="price">Bs {item.price}</span></div><h4>{item.name}</h4>{item.description && <p>{item.description}</p>}<button className="add-button" aria-label={`Agregar ${item.name}`} onClick={() => add(item.name)}><Plus size={16}/></button></article>)}</div></section>)}</div></section>}

    <section id="passport" className="passport page-shell"><div><p className="eyebrow">Kava Pasaporte</p><h2>Tu café,<br/><em>tu ritual.</em></h2><p>Descubre nuevos orígenes, acumula sellos y deja que cada taza te lleve un poco más lejos.</p><button className="button-primary" onClick={openPassport}>Reservar pasaporte <ArrowRight size={15}/></button></div><div className="passport-video-frame"><video className="passport-video" autoPlay muted loop playsInline preload="metadata" aria-label="Experiencia Kava Pasaporte de Café"><source src="/kava-pasaporte.mp4" type="video/mp4" />Tu navegador no admite la reproducción de video.</video></div></section>
  ...
  <div className="location-info">
    <p className="location-address">Visítanos en Kava Eco Coffee</p>
    ...
    <div className="mt-6 overflow-hidden rounded-2xl shadow-md border border-black/10 w-full">
      <iframe
        ...
        title="Ubicación Kava Eco Coffee"
